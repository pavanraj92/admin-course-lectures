                            ...
                            <div class="form-group">
                                ...
                                <div id="attachmentDropzone" class="dropzone">
                                    <div class="dz-message">
                                        ...
                                        <p class="text-muted">Supported formats: JPG, PDF, DOC, DOCX, PPT, PPTX, XLS, XLSX,
                                            ZIP, RAR (Max: 50MB)</p>
                                    </div>
                                </div>
                                <!-- Hidden file input -->
                                <input type="file" id="attachmentFileInput" name="attachment"
                                    accept=".jpg,.jpeg,.pdf,.doc,.docx,.ppt,.pptx,.xls,.xlsx,.zip,.rar"
                                    style="display: none;">
                                @error('attachment')
                                    <div class="text-danger validation-error">{{ $message }}</div>
                                @enderror
        ...
        $(document).ready(function() {
            ...
            $('#lectureForm').validate({
                ...
                rules: {
                    ...
                    video: {
                        required: function() {
                            return $('#type').val() === 'video' && $('#videoPreview .existing-file')
                                .length === 0 && $('#videoPreview .new-file').length === 0;
                        },
                        extension: "mp4|avi|mov|wmv"
                    },
                    audio: {
                        required: function() {
                            return $('#type').val() === 'audio' && $('#audioPreview .existing-file')
                                .length === 0 && $('#audioPreview .new-file').length === 0;
                        },
            ...
            $('#course_id').on('change', function() {
                ...
                if (courseId) {
                    $.ajax({
                        url: "{{ route('admin.courses.sections', ['course' => '__courseId__']) }}"
